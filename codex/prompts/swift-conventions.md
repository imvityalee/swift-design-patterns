---
description: "View or scaffold this project's Swift architecture & conventions file that the swift-design-patterns tools read. Usage: /swift-conventions (view) · /swift-conventions init (create)"
---

The user invoked `/swift-conventions` with: **$ARGUMENTS**

You have the `swift-design-patterns` MCP server connected.
...
- If `$ARGUMENTS` is `init`, `scaffold`, `create`, or `new`:
  ...
  3. Otherwise write the template to `.swift-architecture.md` in the project
     root, then tell the user to fill in the sections (especially
